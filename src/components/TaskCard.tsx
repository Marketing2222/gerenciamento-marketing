...
export default function TaskCard({ task, index, onClick }: TaskCardProps) {
  ...
  const isClose = task.dueDate && !isOverdue && (new Date(task.dueDate).getTime() - new Date().getTime() < 48 * 60 * 60 * 1000) && task.status !== 'DONE' // menos de 48 horas
      ...
      {(provided, snapshot) => (
        ...
        >
          ...
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            {isClose && !isOverdue && (
              <span className="flex items-center gap-0.5 text-[9px] text-amber-600 dark:text-amber-400 font-extrabold uppercase tracking-wider">
                <Clock className="w-2.5 h-2.5" />
                Próximo
              </span>
            )}
          ...
          {/* Título */}
          <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm group-hover:text-blue-600 dark:group-hover:text-blue-400 transition leading-snug mb-3">
            {task.title}
          </h4>
